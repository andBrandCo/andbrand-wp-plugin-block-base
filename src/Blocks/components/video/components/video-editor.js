		...
		<>
			{!hasVideo && showVideoUrl &&
				<MediaPlaceholder
					...
					labels={{
						title: __('Video', 'andbrand-wp-plugin-block-base'),
						instructions: __('Upload a video file or pick one from your media library. You can select multiple video files to have fallbacks with different video formats.', 'andbrand-wp-plugin-block-base'),
					}}
					multiple
					accept={videoAccept}
			...
			{!hasVideo && !showVideoUrl && showPlaceholder &&
				<Placeholder icon={icons.video} label={__('Add a video', 'andbrand-wp-plugin-block-base')}>
					{__('Check the block options in the sidebar', 'andbrand-wp-plugin-block-base')}
				</Placeholder>
			}
