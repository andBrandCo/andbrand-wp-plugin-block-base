...
export const LayoutOptions = (attributes) => {
	...
	return (
		<CollapsableComponentUseToggle
			label={sprintf(__('%s', 'andbrand-wp-plugin-block-base'), label)}
			checked={layoutUse}
		...
		>
			<>
				{showLayoutType &&
					<SimpleVerticalSingleSelect
						label={<IconLabel icon={icons.backgroundTypeAlt} label={__('Layout Type', 'andbrand-wp-plugin-block-base')} />}
				...
				{showLayoutTotalItems &&
					<RangeControl
						label={<IconLabel icon={icons.totalItems} label={__('Maximum number of items to show', 'andbrand-wp-plugin-block-base')} />}
