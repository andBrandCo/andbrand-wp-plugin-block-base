...
export const ColumnOptions = ({ attributes, setAttributes }) => {
	...
	return (
		<PanelBody title={__('Column', 'andbrand-wp-plugin-block-base')}>

			<HelpModal
				type='column'
				buttonLabel={__('How to use a Column?', 'andbrand-wp-plugin-block-base')}
				modalLabel={__('Column', 'andbrand-wp-plugin-block-base')}
			/>
			...
			<Responsive label={<IconLabel icon={icons.order} label={__('Order', 'andbrand-wp-plugin-block-base')} />}>
				{breakpoints.map((keyName, index) => {
					...
					return (
						<Fragment key={keyName}>
							...
							/>
							{index === 0 &&
								<p className='es-decorative-text'>{__('Needs to be set on all columns!', 'andbrand-wp-plugin-block-base')}</p>
							}
						</Fragment>
					);
				})}
			...
			<Responsive label={<IconLabel icon={icons.verticalAlign} label={__('Vertical align', 'andbrand-wp-plugin-block-base')} />}>
				{breakpoints.map((keyName, index) => {
					const point = ucfirst(keyName);
			...
			<Responsive label={<IconLabel icon={icons.horizontalAlign} label={__('Horizontal align', 'andbrand-wp-plugin-block-base')} />}>
				{breakpoints.map((keyName, index) => {
					const point = ucfirst(keyName);
