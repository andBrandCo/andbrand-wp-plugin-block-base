...
export const TabsEditor = (props) => {
  ...
  const { blockClass, activeTab, setActiveTab } = attributes;

  console.log("set active tab", setActiveTab);
  ...
  const tabsAllowedBlocks = checkAttr(
    "tabsAllowedBlocks",
    attributes,
    manifest
  );

  const newTabTemplate = [["seb-block-library/tab", {}, [["core/paragraph"]]]];
  ...
  const tabItems = inner.map((block) => {
    ...
    return (
      <li key={block.tabId} className={classes}>
        <button
          onClick={() => {
          ...
          <RichText
            ...
            onChange={(title) =>
              wp.data
                ...
                .updateBlockAttributes(block.clientId, { tabTitle: title })
            }
  ...
  const addInnerBlock = () => {
    ...
    const block = createBlock("seb-block-library/tab");
    wp.data
      .dispatch("core/block-editor")
      .insertBlock(block, innerCount, clientId);
  };
  ...
  return (
    <div className={blockClass}>
      ...
      <InnerBlocks
        allowedBlocks={
          typeof tabsAllowedBlocks === "undefined" || tabsAllowedBlocks
        }
