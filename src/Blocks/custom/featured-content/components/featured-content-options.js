...
export const FeaturedContentOptions = ({ attributes, setAttributes }) => {
		...
		return (
			<>
				<Notice
					...
					isDismissible={false}
				>
					{__('These options can only be used in a single post. Keep in mind that, by selecting this option, the editor preview may not show the correct items!', 'andbrand-wp-plugin-block-base')}
				</Notice>
				<br />
			</>
		);
	...
	return (
		<PanelBody title={__('Featured content', 'andbrand-wp-plugin-block-base')}>
			{postTypeOptions.length > 0 &&
				<>
					<CustomSelect
						label={<IconLabel icon={icons.file} label={__('Filter by content type', 'andbrand-wp-plugin-block-base')} />}
					...
					{taxonomyOption().length > 0 &&
						<CustomSelect
							label={<IconLabel icon={icons.editOptions} label={__('Filter by taxonomy', 'andbrand-wp-plugin-block-base')} />}
					...
					{featuredContentTaxonomy &&
						<>
							<IconToggle
								...
								label={__('Select specific items', 'andbrand-wp-plugin-block-base')}
								help={__('These options will allow you to manually select items to show.', 'andbrand-wp-plugin-block-base')}
							...
							{useSpecificTerms ?
								<CustomSelect
									label={<IconLabel icon={icons.clipboard}  label={__('Show only specific items', 'andbrand-wp-plugin-block-base')} />}
									help={__('You will preview the latest 30 items, use the search option to find the item you are looking for. If nothing is selected, all items are shown.', 'andbrand-wp-plugin-block-base')}
								...
								<>
									<IconToggle
										...
										label={__('Use term of the current post', 'andbrand-wp-plugin-block-base')}
										help={__('This option allows you to show items in the current post taxonomy. In order for this to work you must select value in the Filter by taxonomy option.', 'andbrand-wp-plugin-block-base')}
					...
					{!featuredContentTaxonomy &&
						<>
							<IconToggle
								icon={icons.visible}
								label={__('Select specific items to show?', 'andbrand-wp-plugin-block-base')}
								checked={useSpecificPosts}
								help={__('This option allows you to select individual items to show.', 'andbrand-wp-plugin-block-base')}
								onChange={() => {
							...
							{useSpecificPosts &&
								<CustomSelect
									label={<IconLabel icon={<BlockIcon iconName='esf-select' />}  label={__('Show only specific items', 'andbrand-wp-plugin-block-base')} />}
									help={__('You will preview the latest 30 items, use the search option to find the item you are looking for. If nothing is selected, all items are shown.', 'andbrand-wp-plugin-block-base')}
			...
			<IconToggle
				icon={icons.none}
				label={__('Exclude the current post', 'andbrand-wp-plugin-block-base')}
				checked={featuredContentExcludeCurrentPost}
				help={__('Exclude the current post from the featured content block items.', 'andbrand-wp-plugin-block-base')}
			...
			<IconToggle
				...
				label={__('Use random order', 'andbrand-wp-plugin-block-base')}
				checked={featuredContentRandomOrder}
				help={__('Show posts in random order.', 'andbrand-wp-plugin-block-base')}
